...
const SelectValue = ({ isOpen, isFocused, isDisabled, size, placeholder, placeholderIcon, ref }: SelectValueProps) => {
    return (
        <AriaButton
            ref={ref}
            className={cx(
                "relative flex w-full cursor-pointer items-center rounded-md bg-background shadow-xs ring-1 ring-border outline-hidden transition duration-100 ease-linear ring-inset",
                (isFocused || isOpen) && "ring-2 ring-primary",
                isDisabled && "cursor-not-allowed bg-accent text-muted-foreground",
            )}
        >
            <AriaSelectValue<SelectItemType>
                className={cx(
                    ...
                    // Icon styles
                    "*:data-icon:size-5 *:data-icon:shrink-0 *:data-icon:text-muted-foreground in-disabled:*:data-icon:text-muted-foreground/50",
                    ...
                    return (
                        <>
                            ...
                            {state.selectedItem ? (
                                ...
                                <p className={cx("text-md text-muted-foreground", isDisabled && "text-muted-foreground/50")}>{placeholder}</p>
                            )}
